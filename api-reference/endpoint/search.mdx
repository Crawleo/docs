---
title: "Search API"
seoTitle: "Crawleo Search API - Real-Time Web Search with SERP Extraction"
description: "Real-time web search API with granular SERP enrichment controls, advanced filtering, page content extraction, and support for multiple output formats. Perfect for RAG pipelines and AI agents."
api: "GET /search"
---

## Overview

The Search API provides real-time web search with optional automatic crawling of search results. It now supports granular control over which SERP enrichments are extracted, giving you precise control over response content. By default, page content formats are disabled, making responses lighter and more predictable.

## Endpoint

```
GET https://api.crawleo.dev/search
```

## Parameters

### Required Headers

<ParamField header="Authorization" type="string" required>
  Bearer token for authentication. Format: `Bearer YOUR_API_KEY`
  
  Example: `Authorization: Bearer YOUR_API_KEY`
</ParamField>

### Required Parameters

<ParamField query="query" type="string" required>
  The search query string.
</ParamField>

### Pagination Parameters

<ParamField query="count" default="10" type="integer">
  Number of results to return per page (1-50).
</ParamField>

<ParamField query="max_pages" default="1" type="integer">
  Maximum number of search result pages to fetch. **Each page costs 10 credits.**
</ParamField>

### Localization Parameters

<ParamField query="setLang" type="string">
  Language code for search results (e.g., `en`, `es`, `fr`, `de`).
</ParamField>

<ParamField query="cc" type="string">
  Country code for search results (e.g., `US`, `GB`, `DE`).
</ParamField>

<ParamField query="geolocation" type="string">
  Geographic location for localized results.
</ParamField>

### Device Simulation

<ParamField query="device" default="desktop" type="string">
  Device type to simulate. Options: `desktop`, `mobile`, `tablet`.
</ParamField>

### SERP Enrichment Toggles

<ParamField query="include_copilot_answers" default="true" type="boolean">
  Include Copilot AI-generated answers in the SERP results.
</ParamField>

<ParamField query="include_people_also_ask" default="true" type="boolean">
  Include "People Also Ask" question cards in the results.
</ParamField>

<ParamField query="include_related_queries" default="true" type="boolean">
  Include related search queries suggestions.
</ParamField>

<ParamField query="include_sidebar" default="true" type="boolean">
  Include sidebar data (knowledge panels, local results, etc.).
</ParamField>

<ParamField query="include_direct_answers" default="true" type="boolean">
  Include direct answer boxes and instant answers.
</ParamField>

### Page Content Format Parameters

<ParamField query="get_raw_html" default="false" type="boolean">
  Return the original HTML source of each result page.
</ParamField>

<ParamField query="get_ai_enhanced_html" default="false" type="boolean">
  Return clean HTML with ads, scripts, and tracking removed.
</ParamField>

<ParamField query="get_page_text" default="false" type="boolean">
  Return plain text content extracted from each result page.
</ParamField>

<ParamField query="get_page_text_markdown" default="false" type="boolean">
  Return content as structured Markdown (recommended for RAG/LLM).
</ParamField>

### Auto-Crawling

<ParamField query="auto_crawling" default="false" type="boolean">
  Automatically crawl each search result URL and include page content.
</ParamField>

## Example Requests

### Basic Search

<CodeGroup>

```bash cURL
curl -X GET "https://api.crawleo.dev/search?query=machine%20learning&count=10" \
  -H "Authorization: Bearer YOUR_API_KEY"
```


```python Python
```


```javascript JavaScript
```

</CodeGroup>

### Search with Auto-Crawling and Markdown Output

<CodeGroup>

```bash cURL
curl -X GET "https://api.crawleo.dev/search?query=python%20tutorials&count=5&auto_crawling=true&get_page_text_markdown=true" \
  -H "Authorization: Bearer YOUR_API_KEY"
```


```python Python
```


```javascript JavaScript
```

</CodeGroup>

### Localized Search

<CodeGroup>

```bash cURL
curl -X GET "https://api.crawleo.dev/search?query=news&setLang=de&cc=DE&count=10" \
  -H "Authorization: Bearer YOUR_API_KEY"
```


```python Python
```

</CodeGroup>

## Response

A successful response returns search results with optional crawled content:

```json
{
  "status": "success",
  "data": {
    "query": "Now in Android",
    "pages_fetched": 1,
    "time_used": 1.2119622230529785,
    "pages": {
      "1": {
        "total_results": "About 41,100 results",
        "search_results": [
          {
            "title": "GitHub - android/nowinandroid: A fully functional Android app built ...",
            "link": "https://github.com/android/nowinandroid",
            "date": null,
            "snippet": "Learn how this app was designed and built in the design case study, architecture learning journey and modularization learning journey.",
            "source": "Github"
          },
          {
            "title": "Now in Android | Android Developers",
            "link": "https://developer.android.com/series/now-in-android",
            "date": "Aug 8, 2025",
            "snippet": "Learn about the latest features and best practices of Android development with Now in Android app and podcast.",
            "source": "Android Developers"
          }
        ],
        "related_queries": [
          "latest Android now",
          "Android now gg",
          "what's new in Android development"
        ],
        "page_content": {
          "enhanced_html": "<body>...</body>",
          "page_markdown": "# Search Results\n..."
        }
      }
    },
    "credits": 10
  }
}
```

<ResponseField name="status" type="string">
  Status of the response (`success` or `error`).
</ResponseField>

<ResponseField name="data" type="object">
  Main data object containing search results and metadata.

  <Expandable title="Data object properties">
    <ResponseField name="query" type="string">
      The search query that was executed.
    </ResponseField>
    <ResponseField name="pages_fetched" type="integer">
      Number of pages fetched from the search engine.
    </ResponseField>
    <ResponseField name="time_used" type="number">
      Time taken to complete the request in seconds.
    </ResponseField>
    <ResponseField name="pages" type="object">
      Object containing results keyed by page number (e.g., "1", "2").

      <Expandable title="Page object properties">
        <ResponseField name="total_results" type="string">
          Estimated total number of results from the search engine.
        </ResponseField>
        <ResponseField name="search_results" type="array">
          Array of search result objects.

          <Expandable title="Search result properties">
            <ResponseField name="title" type="string">
              Title of the search result.
            </ResponseField>
            <ResponseField name="link" type="string">
              URL of the search result.
            </ResponseField>
            <ResponseField name="date" type="string | null">
              Publication date (if available).
            </ResponseField>
            <ResponseField name="snippet" type="string">
              Short description/snippet from the search engine.
            </ResponseField>
            <ResponseField name="source" type="string">
              Source name or domain of the result.
            </ResponseField>
          </Expandable>
        </ResponseField>
        <ResponseField name="related_queries" type="array">
          Array of suggested related search queries.
        </ResponseField>
        <ResponseField name="page_content" type="object">
          Page content (when output format options are enabled).

          <Expandable title="Page content properties">
            <ResponseField name="enhanced_html" type="string">
              Clean HTML with ads/scripts removed (if `get_ai_enhanced_html=true`).
            </ResponseField>
            <ResponseField name="page_markdown" type="string">
              Markdown formatted content (if `get_page_text_markdown=true`).
            </ResponseField>
          </Expandable>
        </ResponseField>
      </Expandable>
    </ResponseField>
    <ResponseField name="credits" type="integer">
      Number of credits consumed by this request.
    </ResponseField>
  </Expandable>
</ResponseField>

## Important Updates

<Callout type="info" title="New SERP Extraction Controls">
  This endpoint now provides granular control over SERP enrichments through five boolean query parameters:
  - `include_copilot_answers` - Control AI-generated answer cards
  - `include_people_also_ask` - Control question suggestions
  - `include_related_queries` - Control related search queries
  - `include_sidebar` - Control sidebar data and knowledge panels
  - `include_direct_answers` - Control instant answer boxes

  Advanced SERP features default to `true`, allowing you to selectively disable them as needed.
</Callout>

<Callout type="warning" title="Page Content Defaults Changed">
  **Breaking Change**: `enhanced_html` and `markdown` now default to `false`. To receive page content, explicitly set `get_ai_enhanced_html=true` and/or `get_page_text_markdown=true` in your request. This makes default responses leaner and more focused on core search results.
</Callout>

## Use Cases

<AccordionGroup>
  <Accordion icon="brain" title="RAG Pipeline">
    Use `auto_crawling=true` with `get_page_text_markdown=true` to fetch search results and their full content in a single request, formatted for LLM consumption.
  </Accordion>
  <Accordion icon="robot" title="Research Agent">
    Combine with localization parameters to gather region-specific information for AI research agents.
  </Accordion>
  <Accordion icon="layer-group" title="Content Aggregation">
    Use `get_ai_enhanced_html=true` to collect clean content from multiple sources without ads and scripts.
  </Accordion>
  <Accordion icon="toggle-left" title="Fine-Grained SERP Control">
    Use the SERP enrichment toggles to customize which advanced search features are extracted. For example, disable `include_people_also_ask=false` if you only need core results and related queries.
  </Accordion>
</AccordionGroup>

in docs/api-reference/endpoint/search.mdx the response isn't correct . fix it . the real output example: